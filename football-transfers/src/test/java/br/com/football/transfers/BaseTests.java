...
public abstract class BaseTests extends CassandraSpringBoot{
    ...
    private ResultHandler buildResultHandler(final String methodName) {
        ...
        if (fieldDescriptors.get("request_" + methodName) != null
                && fieldDescriptors.get("response_" + methodName) != null) {
        ...
        } else if (fieldDescriptors.get("request_" + methodName) != null
                && fieldDescriptors.get("response_" + methodName) == null) {
            return MockMvcRestDocumentation.document(
                    restDocumentation.beforeOperation().getTestMethodName(),
                    requestFields(fieldDescriptors.get("request_" + methodName)));
        } else if (fieldDescriptors.get("response_" + methodName) != null
                && fieldDescriptors.get("request_" + methodName) == null) {
            return MockMvcRestDocumentation.document(
                    restDocumentation.beforeOperation().getTestMethodName(),
                    responseFields(fieldDescriptors.get("response_" + methodName)));
        } else {
